...
### `core/token/uses-a-csprng`

**Kind**: capability
**Claim**: Token bytes come from a cryptographic random source. No weak random source is
reachable in `packages/core`.
...
[C10](../CRUX-FEEDBACK.md) applied to its own example.** The single version wanted a lint witness
and a test witness and neither attested it alone: the test cannot distinguish a CSPRNG from a
good PRNG, so it would affirm a token generated by `Math.random`, and the lint rule says nothing
about length or encoding.

Split, each has one honest witness, and the lint rule is the better of the two — it denies at the
moment the wrong call is written rather than sampling output afterwards.
